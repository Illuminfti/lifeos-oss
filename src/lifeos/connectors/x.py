...
class Plugin:
    """X capture plugin. Outbound send is out of scope."""

    def __init__(self) -> None:
        self.manifest = ConnectorManifest(
            id="org.lifeos.x",
            display_name="X",
            source_classes=['post', 'mention', 'timeline'],
            capabilities=['backfill', 'incremental_sync', 'revoke', 'purge'],
            auth_modes=['oauth'],
            custody="local",
            outbound_actions=False,
            notes="Timelines and mentions. Posting is a separate approval-gated adapter.",
        )
    ...
    def _fixture_event(self) -> CaptureEvent:
        ...
        text = "synthetic fixture; not personal data (x)"
        return CaptureEvent(
            ...
            metadata={"connector": "x", "synthetic": True},
        )
