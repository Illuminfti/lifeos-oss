...
class Plugin:
    """IMAP email capture plugin. Outbound send is out of scope."""

    def __init__(self) -> None:
        self.manifest = ConnectorManifest(
            id="org.lifeos.email-imap",
            display_name="IMAP email",
            source_classes=['message', 'thread', 'attachment'],
            capabilities=['backfill', 'incremental_sync', 'attachments', 'purge'],
            auth_modes=['password'],
            custody="local",
            outbound_actions=False,
            notes="Generic IMAP where the server permits it.",
        )
    ...
    def _fixture_event(self) -> CaptureEvent:
        ...
        text = "synthetic fixture; not personal data (email-imap)"
        return CaptureEvent(
            ...
            metadata={"connector": "email-imap", "synthetic": True},
        )
