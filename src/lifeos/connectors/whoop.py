...
class Plugin:
    """WHOOP capture plugin. Outbound send is out of scope."""

    def __init__(self) -> None:
        self.manifest = ConnectorManifest(
            id="org.lifeos.whoop",
            display_name="WHOOP",
            source_classes=['recovery', 'sleep', 'workout', 'cycle'],
            capabilities=['backfill', 'incremental_sync', 'revoke', 'purge'],
            auth_modes=['oauth'],
            custody="local",
            outbound_actions=False,
            notes="OAuth scopes for recovery, sleep, workout, cycle. Missing data is not zero.",
        )
    ...
    def _fixture_event(self) -> CaptureEvent:
        ...
        text = "synthetic fixture; not personal data (whoop)"
        return CaptureEvent(
            ...
            metadata={"connector": "whoop", "synthetic": True},
        )
